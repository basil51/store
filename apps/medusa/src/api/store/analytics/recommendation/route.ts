...
import {
  ...
  insertRecommendationAnalyticsEvent,
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  const parsed = normalizeRecommendationAnalyticsEvent(req.body)
  ...
  try {
    ...
    await insertRecommendationAnalyticsEvent(db, parsed.value, {
    ...
    logger.error?.("Failed to store recommendation analytics event", {
