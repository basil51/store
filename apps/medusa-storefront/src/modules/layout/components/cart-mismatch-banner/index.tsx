...
function CartMismatchBanner(props: {
  ...
  const [actionText, setActionText] = useState("Sync cart")
  const shouldShowCartSyncBanner = Boolean(customer && !cart.customer_id)

  if (!shouldShowCartSyncBanner) {
    return null
  }
  ...
  const handleSubmit = async () => {
    try {
      ...
      setActionText("Try syncing again")
      setIsPending(false)
    }
  }
    ...
    <div className="flex items-center justify-center small:p-4 p-2 text-center bg-orange-300 small:gap-2 gap-1 text-sm mt-2 text-orange-800">
      <div className="flex flex-col small:flex-row small:gap-2 gap-1 items-center">
        <span className="flex items-center gap-1">
          ...
          This cart is not linked to your account yet
        </span>
