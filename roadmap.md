...
- That coverage now also locks down the normalization guard on search recovery so formatting-only recovery responses do not trigger duplicate fallback fetches or misleading `recovered_query` payloads.
- Phase 15 now also includes focused storefront API coverage for the search analytics proxy route, plus a shared recovery-normalization helper used by both zero-result store search and nav suggestions so formatting-only recovery matches are ignored consistently.
- Phase 15 now also includes focused Medusa route coverage for `/store/analytics/search/recovery`, including empty-query rejection, override-first ordering with duplicate analytics suppression, no-op override filtering when the target normalizes back to the original query, and backend failure logging/response behavior.
- Phase 15 now also includes focused Medusa route coverage for `/store/analytics/search`, including invalid-payload rejection before DB access, request-context forwarding (`host`, `path`, `user-agent`, first `x-forwarded-for` IP with `req.ip` fallback), and the logged 500 persistence-failure path.
- Phase 15 now also includes browser-level storefront coverage for the recovered nav-search flow: Playwright mocks the suggestions API, verifies recovered-query labeling plus recovered product suggestions in the dropdown, and verifies the CTA routes shoppers to the recovered store query.
- Phase 15 now also includes browser-level storefront coverage for the full recovered store-results page flow: Playwright seeds a deterministic recovery override in Postgres, navigates to a zero-result store search, verifies the recovery banner plus CTA copy, and verifies the recovered results list contains the expected product.
- Phase 15 now also includes browser-level storefront coverage for the no-results store-search branch: Playwright drives a guaranteed no-hit query, verifies the empty-state title/subtitle plus clear-search CTA, verifies no recovery banner appears, and verifies the rendered product grid stays empty.
- Phase 15 now also includes browser-level analytics verification for store search result views: Playwright intercepts `/api/analytics/search` and verifies the empty-results branch emits one `search_results_viewed` event while the recovered-results branch emits both the original zero-result view event and the recovered-results follow-up event with `recovery_source`, `recovered_from_query`, and non-zero result count.
- Phase 15 now also includes browser-level analytics verification for the nav no-suggestions dropdown branch: the search form emits one deduped `search_results_viewed` event with source `nav` and `result_count: 0` when quick suggestions stay empty without a recovered query, and Playwright verifies that payload via the intercepted analytics sink.
- Phase 15 now also includes browser-level analytics verification for the recovered-results CTA follow-through: after the shopper clicks the recovery CTA, Playwright verifies the follow-up direct store-results page emits a third `search_results_viewed` event for the recovered query with source `store` and without recovery metadata.
- Phase 15 now also includes focused unit coverage for the nav no-suggestions analytics dedupe helper: the storefront search utilities lock down first-view tracking, duplicate suppression for the same open query, reset-on-close behavior, and re-tracking after reopening the same query.
- Phase 15 now also includes browser-level analytics verification for the recovered nav dropdown branch: the suggestions API exposes `recovery_source`, the nav form suppresses premature empty-view emits during the debounce window, and Playwright verifies the recovered quick-suggestions path emits both the original zero-result nav event and the recovered nav follow-up event with recovery metadata.
- Phase 15 now also includes focused unit coverage for the store results analytics emitter: the storefront helper in `search-results-analytics` locks down the no-query noop case, the direct store-results event shape, the paired recovered-results event sequence, and the fallback to a single direct event when recovery metadata appears without a zero-result origin.
- Phase 15 now also includes focused unit coverage for nav search submission payload shaping: the shared submit helper normalizes whitespace, drops empty submissions, preserves the shared max-length bound, and keeps the emitted payload aligned with the nav search form’s `search_submitted` tracking path.
- Phase 15 now also includes browser-level verification for explicit nav search submission analytics: Playwright submits the header search form with a whitespace-padded single-character query, verifies a single `search_submitted` event is emitted with the normalized query and `source: nav`, and confirms the submit path stays isolated from dropdown view analytics.
- Phase 15 now also includes focused unit coverage for the storefront analytics client wrappers: `trackSearchSubmitted` and `trackSearchResultsViewed` are tested with a frozen clock to verify `occurred_at` wrapping, `nexmart:analytics` dispatch payloads, `dataLayer`/`gtag` emission, and the posted `/api/analytics/search` body for both direct submit and recovered-results view events.
- Phase 15 now also includes focused backend persistence coverage for storefront-originated search events: the shared Medusa insert helper is unit-tested to verify a normalized `search_submitted` event becomes the expected `search_analytics_events` row, including normalized query, original payload, occurred-at timestamp, and captured request metadata.
- Phase 15 now also includes focused backend persistence coverage for recovered result views: the shared Medusa insert helper is unit-tested to verify a `search_results_viewed` recovery event persists `source: store_recovery`, `result_count`, and the full recovery metadata payload (`recovery_source`, `recovered_from_query`, `original_result_count`) alongside the request metadata.
- Phase 15 now also includes focused schema coverage for the shared search analytics storage layer: the Medusa unit spec verifies `ensureSearchAnalyticsTable` and `ensureSearchRecoveryOverrideTable` create the expected columns, defaults, and index names for both analytics events and recovery overrides.
- Phase 15 now also includes focused downstream admin analytics coverage: the Medusa admin search analytics route is unit-tested to verify stored recovery events roll up into the recovery summary split, recovered-query reporting, override list mapping, and recent-query response payloads used by the admin reporting UI.
- Phase 15 now also includes focused admin override lifecycle coverage: the Medusa admin search analytics route is unit-tested for recovery override creation, scoped upsert behavior, normalized-value rejection, required-id validation, successful deletion, and not-found deletion handling.
- Before Phase 16 deployment, the missing storefront contact surface is now closed: Medusa Cart Settings persists a configurable `contact_email` in store metadata with default fallback `info@sparkco.vip`, `/store/store-currency-config` now exposes that email to the storefront, the storefront now has a localized `/contact` page for both support and returns inquiries, and the order-details help links now route to anchored support and returns sections on that new page. Focused validation covers `apps/medusa-storefront/src/lib/data/currency.test.ts` and `apps/medusa/src/api/store/store-currency-config/__tests__/route.unit.spec.ts`.
